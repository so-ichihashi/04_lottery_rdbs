...
ini_set('display_errors',1);
  ...
  // POSTされたパラメータを受け取る
  $myInput = filter_input(INPUT_POST,'input');
  $timestamp = time();

  // クエリを送信しDBに登録
  if (is_null($myInput)) {

    $sql_select = "SELECT * FROM destination ORDER BY id desc LIMIT 10";
    $result_select = $connect -> query($sql_select) or die("SELECTクエリの送信に失敗しました。".$result_select);

  } else {

  $sql_insert = "INSERT INTO destination VALUES(NULL, '$myInput', NOW())";
...
/* $arr = json_encode($destination_list);
